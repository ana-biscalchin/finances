...
import { CategorySelect } from "../shared/CategorySelect.js";
...
export function RecurrencesPage({ selectedMonth, setSelectedMonth }: { selectedMonth: string; setSelectedMonth: (month: string) => void }) {
  const [rules, setRules] = useState<Rule[]>([]); const [forecasts, setForecasts] = useState<Array<Record<string, unknown>>>([]); const [targets, setTargets] = useState<Array<{ value: string; label: string }>>([]); const [categories, setCategories] = useState<Category[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      setError(null);
      const [loadedRules, loadedForecasts, accounts, cards, loadedCategories] = await Promise.all([apiClient.get("/recurrences", recurrenceList), apiClient.get(`/recurrences?month=${selectedMonth}`, z.array(z.record(z.string(), z.unknown()))), apiClient.get("/accounts", targetList), apiClient.get("/credit-cards", targetList), apiClient.get("/categories", categoryList)]);
      setRules(loadedRules); setForecasts(loadedForecasts); setCategories(loadedCategories); setTargets([...accounts.map((item) => ({ value: `account:${item.id}`, label: `Conta · ${item.name}` })), ...cards.map((item) => ({ value: `card:${item.id}`, label: `Cartão · ${item.name}` }))]);
