...
export function BillPaymentPanel({ cardId, billId, accounts, remainingCents, minimumDueCents, minimumMet, status, payments, onChanged }: { cardId: string; billId: string; accounts: Account[]; remainingCents: number; minimumDueCents: number | null; minimumMet: boolean; status: string; payments: Payment[]; onChanged: () => void }) {
  const [accountId, setAccountId] = useState<string | null>(accounts[0]?.id ?? null);
  const [date, setDate] = useState(new Date().toISOString().slice(0, 10));
  const [principal, setPrincipal] = useState(remainingCents / 100);
  const [interest, setInterest] = useState(0);
  const [penalty, setPenalty] = useState(0);
  const [minimum, setMinimum] = useState(minimumDueCents === null ? 0 : minimumDueCents / 100);
  ...
  async function saveMinimum() { try { const response = await fetch(`${import.meta.env.VITE_API_URL ?? "http://localhost:3000"}/credit-cards/${cardId}/bills/${billId}/minimum`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ minimumDueCents: Math.round(minimum * 100) }) }); if (!response.ok) throw new Error("Não foi possível definir o mínimo."); onChanged(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Falha ao definir o mínimo."); } }

  return <Paper withBorder p="md"><Stack><Group justify="space-between"><Text fw={700}>Pagamentos</Text><Text>Saldo {money(remainingCents)} · mínimo {minimumDueCents === null ? "não informado" : minimumMet ? "atendido" : "pendente"} · {status}</Text></Group>{error ? <Alert color="red">{error}</Alert> : null}{payments.every((payment) => Boolean(payment.reversedAt)) ? <Group align="end"><NumberInput label="Pagamento mínimo" value={minimum} onChange={(value) => setMinimum(Number(value) || 0)} min={0}/><Button variant="light" onClick={() => void saveMinimum()}>Definir mínimo</Button></Group> : null}<Group align="end"><Select label="Conta" data={accounts.map((item) => ({ value: item.id, label: item.name }))} value={accountId} onChange={setAccountId}/><BusinessDateInput label="Data" value={date} referenceMonth={date.slice(0, 7)} onChange={setDate}/><NumberInput label="Principal" value={principal} onChange={(value) => setPrincipal(Number(value) || 0)} min={0}/><NumberInput label="Juros" value={interest} onChange={(value) => setInterest(Number(value) || 0)} min={0}/><NumberInput label="Multa" value={penalty} onChange={(value) => setPenalty(Number(value) || 0)} min={0}/><Button onClick={pay}>Registrar</Button></Group>{payments.map((payment) => <Group key={payment.id} justify="space-between"><Text>{payment.paymentDate} · principal {money(payment.principalCents)} · encargos {money(payment.interestCents + payment.penaltyCents)} {payment.reversedAt ? "· revertido" : ""}</Text>{!payment.reversedAt ? <Button size="xs" color="red" variant="subtle" onClick={() => void reverse(payment.id)}>Reverter</Button> : null}</Group>)}</Stack></Paper>;
}
