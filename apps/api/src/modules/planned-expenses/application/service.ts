...
export function createPlannedExpenseService(connection: Connection, ownerId: string) {
  ...
  const parse = (input: unknown, id: string, fallbackSortOrder = 0) => {
    try {
  ...
  return {
      ...
      const count =
        base.budgetMonth && base.subcategoryId
          ? db
              ...
              .all().length
          : 0;
      const line = parse(input, crypto.randomUUID(), count);
    ...
    update(id: string, input: unknown) {
      ...
      const line = parse(input, id, current.sortOrder);
    ...
    remove(id: string) {
      get(id);
      db.delete(plannedExpenses)
        .where(and(eq(plannedExpenses.ownerId, ownerId), eq(plannedExpenses.id, id)))
        .run();
    },
    copy(sourceMonth: string, targetMonth: string) {
      const source = this.list(sourceMonth);
