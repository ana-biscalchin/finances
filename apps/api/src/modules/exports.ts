  ...
  app.get("/export", async (request, reply) => {
    ...
    const [
      ...
      db
        ...
        .where(eq(monthlyBudgetAllocations.ownerId, ownerId)),
      db.select().from(monthlyIncomePlans).where(eq(monthlyIncomePlans.ownerId, ownerId)),
      ...
      db
        .select()
        .from(settings)
        .where(and(eq(settings.ownerId, ownerId), notLike(settings.key, "google_%")))
    ]);
    void ownerTransactions;
    void ownerTransfers;
    void ownerRecurrences;
    void ownerAllocations;
    void ownerIncomePlans;
    void ownerReserves;
    void ownerPayments;
    void ownerSettings;
    void ownerAccounts;
    ...
    reply.header(
      "Content-Disposition",
      `attachment; filename="carteira-export-${Date.now()}.json"`
    );
    ...
    return {
      ...
      data: {
        ...
        recurrences: ownerRecurrences,
        monthlyBudgetAllocations: ownerAllocations,
        monthlyIncomePlans: ownerIncomePlans,
        reserves: ownerReserves,
        billPayments: ownerPayments,
