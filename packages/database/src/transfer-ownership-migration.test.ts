...
const ownershipSql = migrationSql("0005_cuddly_cannonball.sql");
...
describe("transfer ownership migration", () => {
  function legacyDatabase(ownerUsername?: string) {
    const directory = mkdtempSync(resolve(tmpdir(), "finances-transfer-owner-"));
  ...
  it("backfills existing transfers only to the explicitly configured bootstrap user", () => {
    ...
    connection.sqlite
      .prepare(
        "INSERT INTO account_transfers (id, source_account_id, destination_account_id, amount_cents, event_date, description) VALUES (?, ?, ?, ?, ?, ?)"
      )
      .run("transfer", "account", "destination", 1000, "2026-07-10", "Transferência");
    connection.sqlite
      .prepare(
    ...
    expect(
      connection.sqlite
        .prepare("SELECT owner_id FROM account_transfers WHERE id = ?")
        .get("transfer")
  ...
  it("rolls back without changing legacy data when the owner is absent", () => {
    ...
    expect(
      connection.sqlite.prepare("SELECT id, description FROM account_transfers").all()
    ).toEqual([{ id: "transfer", description: "Transferência" }]);
    expect(
      connection.sqlite
        .prepare(
          "SELECT COUNT(*) AS count FROM pragma_table_info('account_transfers') WHERE name = 'owner_id'"
        )
        .get()
    ).toEqual({ count: 0 });
    connection.sqlite.close();
